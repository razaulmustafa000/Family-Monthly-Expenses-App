...
export default function RazaProfessionalLedger() {
  const [members] = useState(["Asia Fayyaz", "Raza", "Baqa", "Arshad (Self)"]);
  ...
  const arshadStats = useMemo(() => {
    ...
    const totalPaidByArshad = transactions
      .filter(t => t.type === 'SETTLED')
      .reduce((a, b) => a + Number(b.amount), 0);

    return {
      totalDebt: totalOwedByArshad,
      totalPaid: totalPaidByArshad,
      remaining: totalOwedByArshad - totalPaidByArshad
    };
  }, [transactions]);
  ...
  const memberStats = useMemo(() => {
    return members.map(m => {
      const spent = transactions
        .filter(t => t.memberName === m && t.type === 'SPENT')
        .reduce((a, b) => a + Number(b.amount), 0);
      
      const settledAgainstMember = transactions
        .filter(t => t.memberName === m && t.type === 'SETTLED')
        .reduce((a, b) => a + Number(b.amount), 0);

      return { 
        name: m, 
        totalSpent: spent, 
        pending: m === "Arshad (Self)" ? 0 : spent - settledAgainstMember 
      };
    });
  ...
  const addEntry = async (type) => {
    ...
    try {
      await addDoc(collection(db, "transactions"), {
        ...
        description: note || (type === 'SPENT' ? 'Daily Expense' : 'Cash Settlement'),
        type: type,
  ...
  const clearDatabase = async () => {
    if (!confirm("Caution: Are you sure you want to clear all data?")) return;
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Unar Systems - Hissab Report", 14, 15);
    const tableData = transactions.map(t => [
      `${new Date(t.date).toLocaleDateString()} ${new Date(t.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}`,
      t.memberName,
      t.description,
      t.type === 'SPENT' ? 'Expense' : 'Paid Back',
      `Rs ${t.amount.toLocaleString()}`
    ]);
    autoTable(doc, {
      startY: 25,
      head: [['Date & Time', 'Member', 'Note', 'Type', 'Amount']],
      body: tableData,
    });
    doc.save("Unar_Systems_Report.pdf");
  };

  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 pb-20 font-sans">
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
             ...
             <div className="leading-tight">
                <h1 className="font-black text-white text-lg tracking-tighter uppercase leading-none">UNAR</h1>
                <p className="text-[9px] text-cyan-500 font-bold tracking-widest uppercase mt-0.5">Systems</p>
             </div>
          </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2 bg-gradient-to-br from-cyan-600 to-blue-700 p-8 rounded-[2rem] shadow-2xl shadow-cyan-900/20">
                <p className="text-cyan-100 text-[10px] font-black uppercase tracking-[0.2em]">Net Pending Amount</p>
                ...
                <div className="mt-8 flex gap-6 border-t border-white/10 pt-6">
                   <div>
                      <p className="text-[9px] font-bold text-cyan-200 uppercase tracking-wider">Total Paid (Arshad)</p>
                      <p className="text-white font-black text-lg">Rs {arshadStats.totalPaid.toLocaleString()}</p>
                   </div>
                   <div className="w-px h-10 bg-white/20"></div>
                   <div>
                      <p className="text-[9px] font-bold text-cyan-200 uppercase tracking-wider">Total Expenses</p>
                      <p className="text-white font-black text-lg">Rs {arshadStats.totalDebt.toLocaleString()}</p>
                   </div>
                </div>
              </div>
              ...
              {memberStats.filter(m => m.name !== "Arshad (Self)").map(m => (
                <div key={m.name} className="bg-slate-900/40 border border-white/5 p-5 rounded-2xl">
                  ...
                  <div className="mt-4 flex justify-between items-end">
                    <div>
                      <p className="text-[9px] font-bold text-slate-500 uppercase">Pending From Arshad</p>
                      <p className="text-lg font-black text-white leading-none">Rs {m.pending.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-slate-900/80 border border-white/10 p-8 rounded-[2.5rem] backdrop-blur-md">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Amount" className="bg-black border border-white/10 rounded-xl p-4 text-white font-bold outline-none focus:border-cyan-500" />
                <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="Note (e.g. Bill, Food)" className="bg-black border border-white/10 rounded-xl p-4 text-white outline-none focus:border-cyan-500" />
              </div>
              <div className="grid grid-cols-2 gap-4 mt-6">
                <button onClick={() => addEntry('SPENT')} className="bg-white text-black py-4 rounded-xl font-black text-xs uppercase hover:bg-rose-500 hover:text-white transition-all shadow-lg">Member Expense</button>
                <button onClick={() => addEntry('SETTLED')} className="bg-slate-800 text-white py-4 rounded-xl font-black text-xs uppercase hover:bg-emerald-500 transition-all border border-white/5 shadow-lg">Arshad Paid Back</button>
              </div>
            </div>
          </>
        )}
                ...
                {transactions.map(t => (
                  <tr key={t.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="p-6 text-[11px] text-slate-500 font-mono">
                      ...
                      <span className="text-cyan-600 font-bold">{new Date(t.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                    </td>
                    <td className="p-6">
                    ...
                    <td className="p-6">
                      <div className="flex flex-col">
                        <span className={`text-sm font-black ${t.type === 'SPENT' ? 'text-rose-500' : 'text-emerald-500'}`}>
                          {t.type === 'SPENT' ? '-' : '+'} Rs {t.amount.toLocaleString()}
                        </span>
                        <span className="text-[8px] uppercase font-black tracking-widest text-slate-600 mt-1">
                          {t.type === 'SPENT' ? 'Expense' : 'Paid Back'}
                        </span>
                      </div>
                    </td>
                    <td className="p-6 text-right">
                      <button onClick={async () => { if(confirm("Delete this entry?")) await deleteDoc(doc(db, "transactions", t.id)) }} className="text-slate-800 hover:text-rose-500 transition-colors p-2"><Trash2 size={18}/></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <footer className="mt-10 flex flex-col items-center gap-2">
         <div className="bg-slate-900 border border-white/5 px-6 py-3 rounded-full flex items-center gap-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em]">
                Architected by <span className="text-white">Unar Systems</span>
            </p>
         </div>
         <p className="text-[8px] text-slate-600 uppercase font-bold tracking-widest">© 2026 Secured Ledger</p>
      </footer>
    </div>
  );
}